/* path: app/api/reports/get/route.ts */
...
export async function POST(req: Request) {
  try {
    ...
    if (!rep) return NextResponse.json({ ok: false, error: 'NOT_FOUND' }, { status: 404 });
    ...
    return NextResponse.json({
      ...
      report: {
        ...
        numMode: rep.numMode ?? null,
        astroMode: rep.astroMode ?? null,
        palmScanId: rep.palmScanId ?? null,
        input: rep.input ?? null,
      },
      text: rep.text ? String(rep.text) : '',
      hasText: Boolean(rep.text) && rep.status === 'READY',
    });
  } catch (e: any) {
